...
int vsprintf(char* out, const char* fmt, va_list ap) { return vsnprintf(out, -1, fmt, ap); }
    ...
    switch (state) {
      ...
      case 1:
        switch (fmt[i]) {
          case 's':
            txt = va_arg(ap, char*);
            for (int k = 0; txt[k] != '\0'; ++k) append(txt[k]);
            break;
          ...
          case 'd':
            ...
            for (int k = len - 1; k >= 0; --k) append(buffer[k]);
            break;
          ...
          case 'p':
            ...
            for (len = 0; pointer; pointer /= 16, ++len) buffer[len] = HEX_CHARACTERS[pointer % 16];  //逆序的
            for (int k = 0; k < BIT_WIDE_HEX - len; ++k) append('0');
          ...
          case 'x':
            ...
            for (len = 0; unum; unum >>= 4, ++len) buffer[len] = HEX_CHARACTERS[unum & 0xF];  //逆序的
