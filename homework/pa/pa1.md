...
riscv32有R、I、S、B、U、J6种指令格式。

- LUI指令的行为是什么?

LUI指令是将20位常量加载到寄存器的高20位。
...
使用命令find . -name "*[.hl.c]" |xargs cat/wc-l得出来的行数是5877。

使用命令find . -name "*[h.c]" |xargs catlgrep -v ^$|we -1过滤空行后，得出的行数是4822。
